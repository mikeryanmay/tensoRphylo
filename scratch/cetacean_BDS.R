...
tp   <- makeTensorPhylo(tree, num_states = NUM_CATS * NUM_CATS)
...
obj <- function(pars) {
  ...
  # H <- matrix( 1 / (NUM_CATS - 1), NUM_CATS, NUM_CATS)
  # diag(H) <- 0
  # H <- eta * H
  ...
  # set the values
  tp$setLambdaStateDependent(lambda_comb)
  tp$setMuStateDependent(mu_comb)
