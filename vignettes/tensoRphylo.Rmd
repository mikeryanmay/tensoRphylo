...
The purpose of this vignette is to show `R` developers how to use `tensoRphylo` in their own packages using the native `R` interface (the "frontend").
The `tensoRphylo` package also provides header files for those interested in linking their `Rcpp` packages to the `C++` `TensorPhylo` ("backend") library; see the <> vignette for more information.
...
All interactions with the `TensorPhylo` backend are mediated through `R` objects of class `TensorPhyloInstance`.
