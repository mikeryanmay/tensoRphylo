...
Welcome to the extended documentation for the `R` package `tensoRphylo`, which is an `R` interface to the `C++` library `TensorPhylo`.
...
The tree **must** be a rooted tree in `ape::phylo` format, with extant **and/or** extant samples.
Trees **may** have sampled ancestors.
In general, `ape::read.nexus` does an excellent job of parsing Newick strings such that they can be used in `tensoRphylo`.

See the example sampled-ancestor tree in the `inst/testdata` directory:
```{r}
# path to tree file
tree_file <- system.file("testdata", "sampled_ancestor_tree.nex",  package = "tensoRphylo")
...
The tree **may** have a root edge, for example:
```{r}
# print the root edge
tree$root.edge
```
in which case `tensoRphylo` will assume the process begins at the starting of this root edge.
Otherwise, `tensoRphylo` will assume the process begin at the root of the tree.
...
plot(tree, no.margin = TRUE, root.edge = TRUE, underscore = TRUE)
