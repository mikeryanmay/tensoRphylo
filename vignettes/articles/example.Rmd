...
In this example, we'll use `tensoRphylo` to fit a birth-death-shift model to the cetacean phylogeny, similar to the analysis [demonstrated here](https://speciationextinction.info/articles/MiSSE-vignette.html).

## Reading data

We start by loading `tensoRphylo` and the cetacean phylogeny (available here).
```{r eval = FALSE}
library(tensoRphylo)

# read the whale tree
tree <- read.tree("whales_Steemanetal2009.tre")

# plot the tree
plot(tree)
```

```{r echo = FALSE, message = FALSE, fig.height = 10, fig.width = 10, out.width = "95%", fig.align = "center", fig.cap = "The cetacean phylogeny from Steeman et al. 2009."}
library(tensoRphylo)
...
# plot the tree
plot(tree, no.margin = TRUE, cex = 0.75)
```

## Specifying the model

First we create the `tensoRphylo::TensorPhyloInstance`.
```{r}

```

We'll assume that speciation and extinction rates evolve according to separate Markov processes.
