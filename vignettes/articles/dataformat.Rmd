...
The data formats used by `tensorPhylo` are fairly standard and intended to require minimal preprocessing: everything we do is native to standard Newick, `NEXUS`, and `ape::phylo` formats.
...
You can find a nexus file with a valid Newick representation of a sampled-ancestor tree in the `inst/testdata` directory:
```{r}
# path to tree file
...
I provide an example of a dataset with this format in the `inst/testdata` directory:
```{r}
...
Ambiguous data is represented as a `/`-delimited string of the possible states.
...
```{r}
# make some ambiguous and missing data
data[1] <- "0/1"
data[2] <- "?"
data[3] <- "-"
...
This function correctly interprets missing data and determines the number of states and the state labels from the `NEXUS` file.
```{r}
# path to character data file
