...
<!-- Here you will find the full documentation for creating and and using `TensorPhyloInstance` objects. -->
<!-- See the [Modeling Framework](articles/dataformat.html) for details about the available models, and the [Quickstart Guide](articles/tensoRphylo.html) for a more brief introduction to the program. -->

# Notation

`tensoRphylo` is very particular about the types and sizes of arguments you provide.
Therefore, throughout this documentation I will use _data types_ to make it clear what `tensoRphylo` expects.
I will indicate the data type and the package of origin like `package::type`.
...
For example, when I mean to indicate that an argument should be a single numeric value (a scalar), I will write `base::numeric(1)`, to indicate that the argument should be a single numeric value.
...
I will use `num_tips` as a stand-in for the number of tips, and `num_states` as a stand-in for the number of states.
...
# Lineage-specific diversification models
...
### State-dependent speciation events
...
### Time-dependent speciation events
...
### Time-and-state-dependent speciation events
...
### State-dependent extinction events
...
### Time-dependent extinction events
...
### Time-and-state-dependent extinction events
...
### State-dependent sampling events
...
### Time-dependent sampling events
...
### Time-and-state-dependent sampling events
...
### Constant destructive-sampling events
...
### State-dependent destructive-sampling events
...
### Time-dependent destructive-sampling events
...
### Time-and-state-dependent destructive-sampling events
...
## Time-dependent state-change models
...
### Constant, symmetrical anagenetic events
...
### Constant, asymmetrical anagenetic events
...
### Time-dependent, symmetrical anagenetic events
...
### Time-dependent, asymmetrical anagenetic events
...
### Mass-extinction-and-anagensis events
...
For example, in `diversitree`, the rate at which a lineage in state _i_ speciates and produces one lineage in state _j_ and the other in state _k_ is denote $\lambda_{ijk}$.
...
### Time-dependent cladogenetic events
...
# Stochastic mapping and branch rates

## Generate a sample of stochastic maps 
...
simmaps <- tp$drawStochasticMaps(reps = base::numeric(1))
```

## Simulate the average diversification rates per branch
...
branch_rates <- tp$drawBranchRates(reps = base::numeric(1))
```
