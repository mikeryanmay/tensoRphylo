...
ParallelBranchwiseCPU::ParallelBranchwiseCPU(Likelihood::Integrator::integrationScheme_t aIntScheme,
		...
		ParallelAsynchApproximator(aIntScheme, aConditionType, aPtrData, aPtrScheduler, aPtrSyncEventsCont, aPtrTensorCont),
...
ParallelBranchwiseCPU::~ParallelBranchwiseCPU() {
}

void ParallelBranchwiseCPU::setDefaultDeltaT(double aDeltaT) {
	deltaT = aDeltaT;
}

size_t ParallelBranchwiseCPU::getTotalNumberOfIntegrationSteps() const {
	return 0;
}

void ParallelBranchwiseCPU::doPreProcessingSteps() {
...
void ParallelBranchwiseCPU::doIntegrationStep(Likelihood::Scheduler::DAG::NodeDAG* task) {
	...
	ptrIntegrator = Likelihood::Integrator::Factory::createIntegrator<stateType_t, intKernel_t, operations_t >(DEFAULT_ABS_TOLERANCE, DEFAULT_REL_TOLERANCE, deltaT, intScheme);
	...
	if(Utils::Output::outputManager().check(Utils::Output::DUMP_VERB)) {
		#pragma omp critical (integration_times)
		{
...
void ParallelBranchwiseCPU::doEventStep(Likelihood::Scheduler::DAG::NodeDAG* task) {
...
void ParallelBranchwiseCPU::doPostProcessingSteps() {
	// Nothing to be done?
...
void ParallelBranchwiseCPU::doReportState(Likelihood::Scheduler::DAG::NodeDAG* task) {

	if(task->getProbStates().empty()) return;

	#pragma omp critical (probes)
	{
