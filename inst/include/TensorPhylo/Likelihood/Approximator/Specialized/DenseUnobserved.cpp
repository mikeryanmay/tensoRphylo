...
DenseUnobserved::DenseUnobserved(
	...
	kernels(BaseDense::PROCESS_TYPE, aPtrSyncEventsCont, aPtrTensorCont),
	intKernel(BaseDense::PROCESS_TYPE, aPtrTensorCont) {
}

DenseUnobserved::~DenseUnobserved() {
}

void DenseUnobserved::doIntegrationStep(double startTime, double endTime) {
	...
	//std::cout << "Integrate from : " << startTime << " to " << endTime << std::endl;
	...
	Likelihood::Integrator::DenseRungeKuttaDOPRI5<StateType, IntegrationKernelsType, OperationType> *ptrDRK =
			dynamic_cast< Likelihood::Integrator::DenseRungeKuttaDOPRI5<StateType, IntegrationKernelsType, OperationType>* >(ptrIntegrator);
	assert(ptrDRK != NULL);
...
void DenseUnobserved::doEventStep(size_t iEvent) {
