...
#ifndef LIKELIHOOD_STATETYPES_OPENMP_EIGENSTATEOPERATIONS_HPP_
#define LIKELIHOOD_STATETYPES_OPENMP_EIGENSTATEOPERATIONS_HPP_
...
struct EigenStateOperations
{
    ...
    struct scale
    {
        ...
        {
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SCALE")
            t1 *= m_alpha1;
        	_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SCALE")
        }
    ...
    struct scale_sum1
    {
        ...
        {
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_1")
			t1.initMult(m_alpha1, t2);
        	_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_1")
        }
    ...
    struct scale_sum2
    {
        ...
        {
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_2")
          	//t1.resize(t2.size());

			size_t nCols = t2.getStateProb().cols();
			size_t nRows = t2.getStateProb().rows();
			...
			if(Utils::Parallel::Manager::getInstance()->useBlockParallelOMP(nCols)) {
				...
				#pragma omp parallel
				{
					...
					for(size_t iT=0; iT<nThreads; ++iT) { // Potential for pragma openmp
						...
						t1.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) =
											m_alpha1*t2.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha2*t3.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]);
					}
				}
			...
			_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_2")
        }
    ...
    struct scale_sum3
    {
        ...
        {
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_3")
          	//t1.resize(t2.size());
				...
				#pragma omp parallel
				{
					...
					for(size_t iT=0; iT<nThreads; ++iT) { // Potential for pragma openmp
						...
						t1.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) =
											m_alpha1*t2.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha2*t3.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha3*t4.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]);
					}
				}
			...
			t1.vecProbToEdgeMapping = t2.vecProbToEdgeMapping;
			t1.vecScaling = t2.vecScaling;
			_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_3")
        }
    ...
    struct scale_sum4
    {
        ...
        {
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_4")
          	//t1.resize(t2.size());
				...
				#pragma omp parallel
				{
					...
					for(size_t iT=0; iT<nThreads; ++iT) { // Potential for pragma openmp
						...
						t1.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) =
											...
											m_alpha2*t3.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha3*t4.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha4*t5.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]);
					}
				}
			...
			t1.vecProbToEdgeMapping = t2.vecProbToEdgeMapping;
			t1.vecScaling = t2.vecScaling;
			_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_4")
    ...
    struct scale_sum5
    {
        ...
        {
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_5")
          	//t1.resize(t2.size());
				...
				#pragma omp parallel
				{
					...
					for(size_t iT=0; iT<nThreads; ++iT) { // Potential for pragma openmp
						...
						t1.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) =
											...
											m_alpha3*t4.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha4*t5.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha5*t6.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]);
					}
				}
			...
			t1.vecProbToEdgeMapping = t2.vecProbToEdgeMapping;
			t1.vecScaling = t2.vecScaling;
			_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_5")
        }
    ...
    struct scale_sum6
    {
        ...
        void operator()( EigenState<conditionalProbType> &t1 , const EigenState<conditionalProbType> &t2 , const EigenState<conditionalProbType> &t3 , const EigenState<conditionalProbType> &t4 , const EigenState<conditionalProbType> &t5 , const EigenState<conditionalProbType> &t6 ,const EigenState<conditionalProbType> &t7) const
        {
        	...
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_6")
				...
				#pragma omp parallel
				{
					...
					for(size_t iT=0; iT<nThreads; ++iT) { // Potential for pragma openmp
						...
						t1.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) =
											...
											m_alpha4*t5.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha5*t6.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha6*t7.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]);
					}
				}
			...
			_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_6")
    ...
    struct scale_sum7
    {
        ...
        void operator()( EigenState<conditionalProbType> &t1 , const EigenState<conditionalProbType> &t2 , const EigenState<conditionalProbType> &t3 , const EigenState<conditionalProbType> &t4 , const EigenState<conditionalProbType> &t5 , const EigenState<conditionalProbType> &t6 , const EigenState<conditionalProbType> &t7 , const EigenState<conditionalProbType> &t8 ) const
        {
        	...
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_7")
        	//t1.resize(t2.size());
				...
				#pragma omp parallel
				{
					...
					for(size_t iT=0; iT<nThreads; ++iT) { // Potential for pragma openmp
						...
						t1.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) =
											...
											m_alpha5*t6.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha6*t7.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha7*t8.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]);
					}
				}
			...
			_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_7")
    ...
    struct scale_sum8
    {
        ...
        void operator()( EigenState<conditionalProbType> &t1 , const EigenState<conditionalProbType> &t2 , const EigenState<conditionalProbType> &t3 , const EigenState<conditionalProbType> &t4 , const EigenState<conditionalProbType> &t5 , const EigenState<conditionalProbType> &t6 , const EigenState<conditionalProbType> &t7 , const EigenState<conditionalProbType> &t8 , const EigenState<conditionalProbType> &t9 ) const
        {
        	...
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_8")
        	//t1.resize(t2.size());
				...
				#pragma omp parallel
				{
					...
					for(size_t iT=0; iT<nThreads; ++iT) { // Potential for pragma openmp
						...
						t1.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) =
											...
											m_alpha6*t7.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha7*t8.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha8*t9.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]);
					}
				}
			...
			_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_8")
        }
    ...
    struct scale_sum9
    {
        ...
        void operator()( EigenState<conditionalProbType> &t1 , const EigenState<conditionalProbType> &t2 , const EigenState<conditionalProbType> &t3 , const EigenState<conditionalProbType> &t4 , const EigenState<conditionalProbType> &t5 , const EigenState<conditionalProbType> &t6 , const EigenState<conditionalProbType> &t7 , const EigenState<conditionalProbType> &t8 , const EigenState<conditionalProbType> &t9 , const EigenState<conditionalProbType> &t10 ) const
        {
        	...
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_9")
        	//t1.resize(t2.size());
				...
				#pragma omp parallel
				{
					...
					for(size_t iT=0; iT<nThreads; ++iT) { // Potential for pragma openmp
						...
						t1.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) =
											...
											m_alpha7*t8.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha8*t9.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha9*t10.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]);
					}
				}
			...
			_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_9")
        }
    ...
    struct scale_sum10
    {
        ...
        void operator()( EigenState<conditionalProbType> &t1 , const EigenState<conditionalProbType> &t2 , const EigenState<conditionalProbType> &t3 , const EigenState<conditionalProbType> &t4 , const EigenState<conditionalProbType> &t5 , const EigenState<conditionalProbType> &t6 , const EigenState<conditionalProbType> &t7 , const EigenState<conditionalProbType> &t8 , const EigenState<conditionalProbType> &t9 , const EigenState<conditionalProbType> &t10 , const EigenState<conditionalProbType> &t11 ) const
        {
        	...
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_10")
        	//t1.resize(t2.size());
				...
				#pragma omp parallel
				{
					...
					for(size_t iT=0; iT<nThreads; ++iT) { // Potential for pragma openmp
						...
						t1.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) =
											...
											m_alpha8*t9.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha9*t10.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha10*t11.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]);
					}
				}
			...
			_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_10")
        }
    ...
    struct scale_sum11
    {
        ...
        void operator()( EigenState<conditionalProbType> &t1 , const EigenState<conditionalProbType> &t2 , const EigenState<conditionalProbType> &t3 , const EigenState<conditionalProbType> &t4 , const EigenState<conditionalProbType> &t5 , const EigenState<conditionalProbType> &t6 , const EigenState<conditionalProbType> &t7 , const EigenState<conditionalProbType> &t8 , const EigenState<conditionalProbType> &t9 , const EigenState<conditionalProbType> &t10 , const EigenState<conditionalProbType> &t11 , const EigenState<conditionalProbType> &t12 ) const
        {
        	...
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_11")
        	//t1.resize(t2.size());
				...
				#pragma omp parallel
				{
					...
					for(size_t iT=0; iT<nThreads; ++iT) { // Potential for pragma openmp
						...
						t1.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) =
											...
											m_alpha9*t10.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha10*t11.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha11*t12.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]);
					}
				}
			...
			_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_11")
        }
    ...
    struct scale_sum12
    {
        ...
        void operator()( EigenState<conditionalProbType> &t1 , const EigenState<conditionalProbType> &t2 , const EigenState<conditionalProbType> &t3 , const EigenState<conditionalProbType> &t4 , const EigenState<conditionalProbType> &t5 , const EigenState<conditionalProbType> &t6 , const EigenState<conditionalProbType> &t7 , const EigenState<conditionalProbType> &t8 , const EigenState<conditionalProbType> &t9 , const EigenState<conditionalProbType> &t10 , const EigenState<conditionalProbType> &t11 , const EigenState<conditionalProbType> &t12 , const EigenState<conditionalProbType> &t13 ) const
        {
        	...
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_12")
        	//t1.resize(t2.size());
				...
				#pragma omp parallel
				{
					...
					for(size_t iT=0; iT<nThreads; ++iT) { // Potential for pragma openmp
						...
						t1.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) =
											...
											m_alpha10*t11.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha11*t12.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha12*t13.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]);
					}
				}
			...
			_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_12")
        }
    ...
    struct scale_sum13
    {
        ...
        void operator()( EigenState<conditionalProbType> &t1 , const EigenState<conditionalProbType> &t2 , const EigenState<conditionalProbType> &t3 , const EigenState<conditionalProbType> &t4 , const EigenState<conditionalProbType> &t5 , const EigenState<conditionalProbType> &t6 , const EigenState<conditionalProbType> &t7 , const EigenState<conditionalProbType> &t8 , const EigenState<conditionalProbType> &t9 , const EigenState<conditionalProbType> &t10 , const EigenState<conditionalProbType> &t11 , const EigenState<conditionalProbType> &t12 , const EigenState<conditionalProbType> &t13 , const EigenState<conditionalProbType> &t14 ) const
        {
        	...
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_13")
           	//t1.resize(t2.size());
				...
				#pragma omp parallel
				{
					...
					for(size_t iT=0; iT<nThreads; ++iT) { // Potential for pragma openmp
						...
						t1.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) =
											...
											m_alpha11*t12.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha12*t13.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha13*t14.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]);
					}
				}
			...
			_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_13")
        }
    ...
    struct scale_sum14
    {
        ...
        {
        	_START_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_14")
				...
				#pragma omp parallel
				{
					...
					for(size_t iT=0; iT<nThreads; ++iT) { // Potential for pragma openmp
						...
						t1.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) =
											...
											m_alpha12*t13.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha13*t14.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]) +
											m_alpha14*t15.getStateProb().block(0, chunks[iT], nRows, chunks[iT+1]-chunks[iT]);
					}
				}
			...
			_END_EVENT(Utils::Profiling::UniqueProfiler::getInstance(), "SUM_SCALE_14")
        }
...
#endif /* LIKELIHOOD_STATETYPES_OPENMP_EIGENSTATEOPERATIONS_HPP_ */
