...
# read the results
results <- read.table("results/approximator_results.tsv", header = TRUE, stringsAsFactors = FALSE, sep = "\t")
results$numStates <- factor(results$nstates)
results$numTips   <- factor(results$ntips)
results$algo      <- factor(results$algo, levels = names(tensoRphylo::approximatorVersion[1:3]))

# define the colors
colors <- brewer.pal(5, "Set1")[c(5,2,3)]
names(colors) <- names(tensoRphylo::approximatorVersion[1:3])
...
pdf("figures/approximator_results_time.pdf", height = 5, width = 8)
plot(p)
dev.off()
...
pdf("figures/approximator_results_error.pdf", height = 5, width = 8)
plot(p)
dev.off()
