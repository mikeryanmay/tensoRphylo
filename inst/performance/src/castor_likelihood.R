castor_musse_likelihood <- function(tree,
  ...
  birth_rate_indices		= castor:::get_rate_index_vector(Nstates, birth_rate_model)$index_vector
  death_rate_indices		= castor:::get_rate_index_vector(Nstates, death_rate_model)$index_vector
  ...
  if(!is.null(tip_pstates)){
    ...
    unknown_tips = castor:::get_complement(Ntips, known_tips)
    ...
    unknown_priors = castor:::get_complement(Ntips, known_priors)
  ...
  if(all(sampling_rates==0)){
    ...
    if(!is.null(tip_pstates)){
      ...
      known_extant_priors = castor:::get_intersection(Ntips,known_priors,extant_tips)
      if(ncol(tip_priors)==NPstates){
  ...
  if(is.null(tip_priors)){
    ...
    if(Nknown_tips>0){
      if(length(Psampled_tips)>0){
        ...
        known_Psampled_tips				= castor:::get_intersection(Ntips,known_tips,Psampled_tips)
    ...
    if(length(unknown_tips)>0){
      if(length(Psampled_tips)>0){
        unknown_Psampled_tips 				= castor:::get_intersection(Ntips,unknown_tips,Psampled_tips)
      ...
      if(length(extant_tips)>0){
        unknown_extant_tips				 = castor:::get_intersection(Ntips,unknown_tips,extant_tips)
    ...
    if(length(unknown_priors)>0){
      ...
      if(length(Psampled_tips)>0){
        unknown_Psampled_priors					= castor:::get_intersection(Ntips,unknown_priors,Psampled_tips)
        likelihood_per_pstate					= sampling_rates*(1-reveal_fractions)
      ...
      if(length(extant_tips)>0){
        unknown_extant_priors 				= castor:::get_intersection(Ntips,unknown_priors,extant_tips)
        likelihood_per_pstate 				= sampling_fractions*(1-reveal_fractions)
  ...
  provided_param_values 	= castor:::compress_musse_params(transition_matrix, birth_rates, death_rates, Nstates, transition_indices, birth_rate_indices, death_rate_indices);
  ...
  param_mins = castor:::compress_musse_params(lower$transition_matrix, lower$birth_rates, lower$death_rates, Nstates, transition_indices, birth_rate_indices, death_rate_indices)
  param_mins[is.na(param_mins) | is.nan(param_mins) | (param_mins<0)] = 0;
  param_maxs = castor:::compress_musse_params(upper$transition_matrix, upper$birth_rates, upper$death_rates, Nstates, transition_indices, birth_rate_indices, death_rate_indices)
  ...
  first_guess_compr = castor:::compress_musse_params(first_guess$transition_matrix, first_guess$birth_rates, first_guess$death_rates, Nstates, transition_indices, birth_rate_indices, death_rate_indices)
  ...
  first_guess = castor:::uncompress_musse_params(first_guess_compr, Nstates, transition_indices, birth_rate_indices, death_rate_indices, NULL)
  ...
  if(any(is.na(first_guess$transition_matrix))){
    ...
    guessQ = castor:::guesstimate_Mk_rate_via_independent_contrasts(tree, Nstates=Nstates, tip_states=NULL, tip_priors=tip_priors, allow_ties=TRUE)
    if(guessQ$success){
  ...
  first_guess 	  = castor:::uncompress_musse_params(first_guess_compr, Nstates, transition_indices, birth_rate_indices, death_rate_indices, state_names)
  ...
  param_scales = castor:::compress_musse_params(param_scales$transition_matrix, param_scales$birth_rates, param_scales$death_rates, Nstates, transition_indices, birth_rate_indices, death_rate_indices)
  ...
  objective_function = function(param_values, trial){
    results = tryCatch({ castor:::get_MuSSE_loglikelihood_CPP(	Ntips 							= Ntips,
                                                      Nnodes							= Nnodes,
                                                      Nedges							= Nedges,
                                                      Nstates							= Nstates,
                                                      oldest_age						= oldest_age,
