...
ntips   <- 2^(5:10)
nstates <- 2^(1:5)
algos   <- unlist(tensoRphylo::integrationScheme)
reps    <- 1:100
calcs   <- 10
...
results <- do.call(rbind, mclapply(1:nrow(all_combinations), function(i) {
  ...
  this_algo      <- row[["algos"]]
  this_rep       <- row[["rep"]]
  this_algo_name <- names(tensoRphylo::integrationScheme[this_algo + 1])
...
# write the results
write.table(results, file = "results/integrator_results.tsv", sep = "\t", quote = FALSE, row.names = FALSE)
